...
with h5py.File(injfile, "r") as f:
    ...
    # calculate overlap between hp and hc
        ...
        time1 = timeit.timeit(lambda: overlap(hp, hc, psd=psd, low_frequency_cutoff=f_low, normalized=True), number=1000)/1000
        ...
        time2 =  timeit.timeit(lambda: match(hp, hc, psd=psd, low_frequency_cutoff=f_low), number=1000)/1000
        print("overlap time:", time1)
        print("match time:", time2)
...
#python3 -m cProfile -s time run_inj_test_time.py injection.hdf 0 1 result_time_test.hdf
#with snakeviz
#python3 -m cProfile -o profile.out run_inj_test_time.py injection.hdf 0 1 result_time_test.hdf
